...
\begin{table}
    ...
    \begin{tabular}{|l|l|l|l|}
        \hline
        \textbf{方法} & \textbf{URL} & \textbf{请求参数名及其类型与作用} & \textbf{备注} \\
        \hline
        POST & /api/object\_detection & image（图片，通过文件上传提供）、text\_prompt（文本提示，用于指导模型检测与文本相关对象） & 视觉场景理解服务接口 \\
        \hline
    \end{tabular}
    }
    \label{tab:api_list}
...
最终测试结果见表\ref{tab:test_result}。在15个并发用户下，系统响应整体较快，能满足大多数用户对视觉问答的需要。TPS表现整体稳定，错误率也比较低。总体而言，
